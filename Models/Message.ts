...
export default class Message extends Helper {
    ...
    async fetchDialogs(req, res) {
        try {
            let contacts: WLContactInterface[] = await this.WLredis.getContacts(res.locals.sessionId);
    ...
    async sendDisappearing(req, res) {
        ...
        try {
            const result = await this.session.sendMessage(this.target, { text: req.body.body }, { ephemeralExpiration: WA_DEFAULT_EPHEMERAL })
            this.response(res, 200, true, 'The message has been successfully sent.', result)
        } catch (error) {
